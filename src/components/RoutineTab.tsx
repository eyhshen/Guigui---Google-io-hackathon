...
import { Icon, Button, TextField, EvidenceTag, SegmentedControl, BottleGlyph } from '../np/ui';
import { routineSteps, categoryLabels } from '../data';
import { AccountPromptCard } from './prompts';
import { Product, TravelResult } from '../types';

const TONE_CHIP: Record<string, { c: string; b: string; bg: string }> = {
...
function RoutineTimeline({ inventory }: { inventory: Product[] }) {
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {prod ? (
                  <div style={{ marginTop: 7, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, background: "var(--bg-2)", border: "1px solid var(--line)", borderRadius: 12, padding: "9px 12px" }}>
                    ...
                    <Icon name="CheckSquare" size={14} style={{ color: "var(--mint)", flex: "none" }} />
                  </div>
                ) : (
...
function TravelPacking({ inventory, travelRes, loading, onTravel }: { inventory: Product[]; travelRes: TravelResult | null; loading: boolean; onTravel: (dest: string, days: number) => void }) {
  const submit = (e: React.FormEvent<HTMLFormElement>) => { e.preventDefault(); const fd = new FormData(e.currentTarget); onTravel(fd.get("destination") as string, parseInt(fd.get("days") as string, 10) || 7); };
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <form onSubmit={submit} style={{ background: "var(--surface)", border: "1px solid var(--line)", borderRadius: "var(--r-sheet)", padding: 18, display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <Button type="submit" disabled={loading}><span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}><Icon name="Plane" size={16} /> {loading ? "规划中…" : "生成打包清单"}</span></Button>
      </form>
      {travelRes && (
            ...
            {travelRes.selectedIds.map((id) => {
              ...
              return p ? (
                <div key={id} style={{ display: "flex", alignItems: "center", gap: 11, background: "var(--bg-2)", border: "1px solid var(--line)", borderRadius: 14, padding: "10px 12px" }}>
                  ...
                  <Icon name="Check" size={15} style={{ color: "var(--mint)", marginLeft: "auto", flex: "none" }} />
                </div>
              ) : null;
            })}
          </div>
...
export function RoutineTab({ inventory, sub, setSub, travelRes, loading, onTravel, accountCopy, onOpenAccount, onDismissAccount }: { inventory: Product[]; sub: 'routine' | 'travel'; setSub: (v: 'routine' | 'travel') => void; travelRes: TravelResult | null; loading: boolean; onTravel: (dest: string, days: number) => void; accountCopy: { title: string; description: string } | null; onOpenAccount: () => void; onDismissAccount: () => void }) {
  return (
    <div style={{ flex: 1, minHeight: 0, overflowY: "auto", display: "flex", flexDirection: "column", gap: 14, padding: "6px 20px 8px" }}>
      ...
      <SegmentedControl value={sub} onChange={(v) => setSub(v as 'routine' | 'travel')} options={[{ value: "routine", label: "每日护肤日常" }, { value: "travel", label: "出行智能收纳" }]} />
