...
function ShelfieExplore({ onAdd }) {
  const { BottleGlyph } = DS_exp;
  return (
    <div style={{ flex: 1, minHeight: 0, overflowY: "auto", display: "flex", flexDirection: "column", gap: 16, padding: "6px 20px 8px" }}>
      ...
      <div style={{ position: "relative", overflow: "hidden", borderRadius: "var(--r-sheet)", border: "1px solid var(--line)", background: "var(--surface)", padding: 18, flexShrink: 0 }}>
        ...
        <div style={{ position: "relative", maxWidth: "68%" }}>
          ...
          <div style={{ fontFamily: "var(--font-display, inherit)", fontSize: 17, fontWeight: 200, letterSpacing: ".02em", color: "var(--ink)", marginTop: 10, lineHeight: 1.45 }}>探索最契合你肤质的<b style={{ fontWeight: 700, background: "var(--prism)", WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent" }}>护肤好物</b></div>
        ...
        {D_exp.curatedProducts.map((p) => (
          <div key={p.id} style={{ position: "relative", display: "flex", gap: 14, background: "var(--surface)", border: "1px solid var(--line)", borderRadius: "var(--r-card)", padding: 14, overflow: "hidden" }}>
            ...
            <div style={{ flex: 1, minWidth: 0, paddingRight: 30 }}>
              ...
              <div style={{ fontSize: 10.5, color: "var(--muted)", marginTop: 3, lineHeight: 1.55, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{p.description}</div>
            ...
            <button onClick={() => onAdd(p)} aria-label="添加至化妆柜" style={{ position: "absolute", right: 14, top: "50%", transform: "translateY(-50%)", width: 34, height: 34, borderRadius: 999, background: "var(--prism)", color: "var(--on-prism)", border: "none", cursor: "pointer", display: "grid", placeItems: "center", boxShadow: "var(--glow-prism)" }}><window.Icon name="Plus" size={17} /></button>
          </div>
        ))}
      </div>
    </div>
...
Object.assign(window, { ShelfieExplore });
