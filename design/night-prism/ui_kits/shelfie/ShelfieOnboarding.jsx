...
function ObQ({ children, override }) {
  return (
    <div style={{
      fontFamily: "var(--font-display, inherit)", fontSize: "var(--display-size-ob)",
      fontWeight: 200, letterSpacing: ".05em", lineHeight: 1.4, color: "var(--ink)",
...
function Prism({ children }) {
  return <b style={{ fontWeight: 700, background: "var(--prism)", WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent" }}>{children}</b>;
}

/* prism-selectable option tile. tone="rose" flags caution instead of celebrating. */
function ObTile({ selected, tone = "prism", wide, onClick, children }) {
  const roseSel = tone === "rose" && selected;
  return (
    <button aria-pressed={selected} onClick={onClick} style={{
      font: "inherit", cursor: "pointer", borderRadius: "var(--r-opt)", padding: "14px 6px",
      textAlign: "center", fontSize: 14.5, fontWeight: selected ? 700 : 500, minHeight: 52,
      transition: ".18s", gridColumn: wide ? "1 / -1" : undefined, WebkitTapHighlightColor: "transparent",
...
function ShelfieOnboarding({ onDone, onSkip, toast, introStyle }) {
  const { ScreenHeader, Button, Card } = DS_ob;
  const D = window.ShelfieData;
  const STEPS = ["intro", "skinType", "concerns", "actives", "city", "safety", "done"];
  const [step, setStep] = React.useState(0);
  const [data, setData] = React.useState({ skinType: null, concerns: [], actives: [], sensitive: false, city: null, safety: [] });
  const [stOpen, setStOpen] = React.useState(false);
  const [st, setSt] = React.useState({});
  const key = STEPS[step];

  const gridWrap = { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 9, marginTop: 20, position: "relative", zIndex: 2 };
  const why = (t) => <div style={{ fontSize: 12, color: "var(--muted)", lineHeight: 1.8, marginTop: 9, position: "relative", zIndex: 2 }}>{t}</div>;
  ...
  if (key === "intro") {
    body = (
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 28px", textAlign: "center", position: "relative", zIndex: 2 }}>
        <img src="../../assets/guigui-logo.png" alt="柜柜" style={{ height: 120, width: "auto", display: "block", margin: "0 auto 26px", filter: "drop-shadow(0 0 28px rgba(199,168,240,.55))" }} />
    ...
    body = (
      <div style={{ padding: "22px 20px 0" }}>
        ...
        <div style={gridWrap}>
          {D.OB.skinType.map((o) => <ObTile key={o} selected={g === o} onClick={() => setData((d) => ({ ...d, skinType: o }))}>{o}</ObTile>)}
        </div>
    ...
    body = (
      <div style={{ padding: "22px 20px 0" }}>
        ...
        <div style={gridWrap}>
          {D.OB.concerns.map((o) => <ObTile key={o} selected={data.concerns.includes(o)} onClick={() => toggleMulti("concerns", o, 2)}>{o}</ObTile>)}
        </div>
    ...
    body = (
      <div style={{ padding: "22px 20px 0" }}>
        ...
        <div style={gridWrap}>
          {D.OB.actives.map((o) => <ObTile key={o} selected={data.actives.includes(o)} onClick={() => toggleMulti("actives", o)}>{o}</ObTile>)}
        </div>
    ...
    body = (
      <div style={{ padding: "22px 20px 0" }}>
        ...
        <div style={gridWrap}>
          {D.OB.cities.map((o) => <ObTile key={o} selected={data.city === o} onClick={() => setData((d) => ({ ...d, city: o }))}>{o}</ObTile>)}
        </div>
        {w && (
          <div style={{ marginTop: 16, borderRadius: "var(--r-card-lg)", padding: "15px 16px", background: "var(--surface)", border: "1px solid var(--line)", position: "relative", zIndex: 2, overflow: "hidden" }}>
            <div style={{ position: "absolute", top: -30, right: -30, width: 130, height: 130, borderRadius: "50%", background: "radial-gradient(circle, rgba(245,195,119,.35), transparent 70%)" }}></div>
    ...
    body = (
      <div style={{ textAlign: "center", padding: "26px 20px 0", position: "relative", zIndex: 2 }}>
        <ScreenHeader eyebrow="PROFILE READY" title="档案" em="建好了。" />
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "center", marginTop: 18 }}>
          ...
          {data.actives.filter((a) => !a.includes("没有")).length > 0 && <SumChip>在用:{data.actives.filter((a) => !a.includes("没有")).join("、")}</SumChip>}
          {data.sensitive && <SumChip tone="rose">易敏感</SumChip>}
          {w && <SumChip tone="warm">{data.city} {w.temp}° · {w.tag}</SumChip>}
        </div>
        <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 18, lineHeight: 1.9 }}>你的架子(8 件)就是推荐候选池。<br />皮肤有状况时,去「该用哪瓶」。</div>
    ...
    <React.Fragment>
      <div style={{ padding: "14px 20px 0", position: "relative", zIndex: 2, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ flex: 1, height: 3, borderRadius: 99, background: "var(--surface-2)", margin: "0 14px", overflow: "hidden" }}>
          <i style={{ display: "block", height: "100%", background: "var(--prism)", borderRadius: 99, transition: "width .35s ease", width: pct + "%" }}></i>
        </div>
    ...
    <React.Fragment>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
        {a.map((label, i) => {
          ...
          return <button key={label} aria-pressed={on} onClick={() => onPick(i)} style={{
            font: "inherit", flex: 1, fontSize: 12, borderRadius: 999, padding: "8px 0", minHeight: 38, cursor: "pointer",
            border: `1px solid ${on ? "var(--blue)" : "var(--line)"}`, color: on ? "var(--ink-on-blue)" : "var(--ink-soft)",
            background: on ? "rgba(146,196,242,.18)" : "none", fontWeight: on ? 700 : 500,
          }}>{label}</button>;
        })}
      </div>
    </React.Fragment>
...
function SumChip({ tone, children }) {
...
Object.assign(window, { ShelfieOnboarding });
