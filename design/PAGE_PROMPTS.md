...
>
> Legend: 🟢 stable in Batch 1 · 🟣 from the 夜虹 mockup · 🔵 to add (from GuiGui `main`)
...
- Skin type (single-select): 干性/油性/混合/敏感/中性 as prism-selectable tiles,
  plus a collapsible "不确定?自测 2 题" self-test (紧绷? 出油?) that infers a type.
- Concerns (multi, max 2): 冒痘/泛红/干燥/熬夜后/上妆前/换季偏干. Note when 泛红
  is picked ("后面会默认帮你避开酸类/A醇").
...
## 8 · Explore / 该补什么 🔵  `发现·该补什么`  *(add — from GuiGui `main`)*
...
## 9 · Travel / 出行收纳 🔵  `出行·智能收纳`  *(add — from GuiGui `main`)*
...
| 该用哪瓶 · intro | 夜虹 | — |
| 该用哪瓶 · result | 夜虹 | — |
| Explore · 该补什么 | GuiGui `main` | 🔵 add |
| Travel · 出行收纳 | GuiGui `main` | 🔵 add |
| Product sheet | 夜虹 | — |
| Profile · 肤质档案 | GuiGui `main` | 🔵 optional |
